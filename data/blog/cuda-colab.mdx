...
I tried to go one step further, and wrote a simple CUDA kernel for vector addition kernel with some error handling to understand what was going on. Trying to run this with `nvcc4jupyter`'s `%cuda` magic command gave me the following response: `CUDA error in /tmp/tmpjnowjf_2/b1cdcef3-2dc4-4ef4-94e4-f2cd3aae3655/single_file.cu at line 66: the provided PTX was compiled with an unsupported toolchain.` Perhaps, there is [no free lunch](https://en.wikipedia.org/wiki/No_free_lunch_theorem) when it comes to accessing GPUs on the cloud as well! 

> **tl;dr** If you want to skip the debugging journey and get straight to the working solution, jump to the [Moving Forward](#moving-forward) section.

# Identifying the Problem

Let me actually take a step back, and walk you through my process of trying to get CUDA working on Colab, and how I identified the underlying problem. This will be completely trivial for someone familiar with CUDA and NVCC (NVIDIA CUDA Compiler), but will probably be insightful for machine learning practitioners who have only interacted with CUDA through high-level libraries.

## Attempt #1: Using `nvcc4jupyter`

The top result from Google when searching for "Run cuda on colab" is a [discussion on NVIDIA developer forums](https://forums.developer.nvidia.com/t/running-c-and-cuda-in-google-colab/237108) from 2022, which links to this [StackOverflow discussion](https://stackoverflow.com/questions/56854243/how-to-link-the-libraries-when-executing-cuda-program-on-google-colab/56908350#56908350) which uses the `nvcc4jupyter` and is very similar to what I just did. No luck here!
...
At this point, I looked up Issues on the [`nvcc4jupyter` Github repository](https://github.com/andreinechaev/nvcc4jupyter) and came across [Issue #40](https://github.com/andreinechaev/nvcc4jupyter/issues/40):
...
So it seems to me as an outsider that if you compiled your CUDA code with the right flags, and already have the compiled binary available, it should be able to run on an older driver of the same CUDA release family for CUDA 12.x or CUDA 11.x. This was confirmed when I read about the CUDA compilation process (discussed in the next section). But for our case, where we want to write and compile CUDA code ourself, it is important to match the toolkit version with the driver version.

As an aside, I also found out that Google Colab has an active Github organization where they detail CUDA version upgrades to their Colab VM via [lable=announcement issues](https://github.com/googlecolab/colabtools/issues?q=is%3Aissue%20state%3Aclosed%20cuda%20label%3Aannounce%20). It seems like the CUDA-12.5 upgrade happened just a few days before this blog was written! Oh, and a bunch of folks are ranting about how the upate broke stuff for them in the [CUDA 12.5 upgrade issue comments](https://github.com/googlecolab/colabtools/issues/5061) ;)
...
Now, as I was trying to pinpoint the issue the `nvcc4jupyter`, I learned that I could get away without downgrading my CUDA version for the free tier T4 GPU on Colab. Understanding why requires a slight detour into NVIDIA's GPU compute capabilities and how nvcc can control the machine code generated.  
...
but I kept running the same error. It seemed like `nvcc4jupyter` had issues parsing and passing compiler arguments to the underlying nvcc compiler. A second visit to the `nvcc4jupyter` issues page confirmed my suspicions, as the only other open issue [Issue #39](https://github.com/andreinechaev/nvcc4jupyter/issues/39) was:
...
But as I read through the comments and the argument parsing code in `nvcc4jupyter`, I realized that this is inconsistent and not a proper solution to the underlying issue, which appears to be how the `nvcc4jupyter` parser handles arguments containing options like `-l` (for linking libraries) or `-arch` (for specifying the architecture). Actually, going through the package code made me realize that the package is a very lightweight wrapper to making nvcc calls, which led me on to my next attempt.

## Attempt #3: Using NVCC Directly

Since `nvcc4jupyter` wasn't cooperating, I decided to try compiling and running my CUDA code directly using the nvcc compiler that comes pre-installed with Colab. One of the really nice aspects of jupyter notebooks is the ability to run bash commands directly using the `!` prefix. And since we have access to the command line, we can (among other things) write our CUDA code to a file, compile it using nvcc, and run the resulting executable. So, I started by writing out my vector addition code to a file:
...
First, I tried compiling without specifying any architecture, just as a sanity check to verify that the error I saw with `nvcc4jupyter` magic commands was replicated in calling nvcc directly:
